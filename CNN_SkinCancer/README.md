...
### Option 1: Use Pre-Trained Model (Recommended - No Training Required!)

A pre-trained model (70 epochs) is available for immediate use:

```bash
python -m venv venv
source venv/bin/activate
pip install -r requirements.txt
streamlit run app_pretrained.py
```

The pre-trained app (`app_pretrained.py`) loads the already-trained model and focuses on:
- Loading the pre-trained model (70 epochs)
- Downloading the dataset
- Making predictions on test images
- Viewing model performance and training history

**Requirements**: Ensure `trained_model_70epochs.pkl` exists in the project directory.

### Option 2: Train from Scratch

To train a new model from scratch:

1. **Interactive training** (Streamlit UI with custom hyperparameters):
```bash
streamlit run app.py
```

2. **Command-line training** (70 epochs, automated):
```bash
# Use caffeinate to prevent Mac from sleeping during training
caffeinate -i python train_70_epochs.py
```

The Streamlit UI (`app.py`) walks you through downloading the dataset automatically (via `kagglehub`), loading/preprocessing data, creating the CNN model and training it interactively.
...
- `app_pretrained.py` — **Streamlit app for pre-trained model** (loads trained model, makes predictions, shows training history)
- `app.py` — Main Streamlit application for training from scratch with UI controls (download dataset, set hyperparameters, run training)
- `train_70_epochs.py` — **Automated training script** that trains the model for 70 epochs and saves as pickle file (can be run with `caffeinate` to prevent sleep)
- `streamlit_frontpage.py` — Enhanced informational front page explaining the model architecture, dataset, and usage
...
- `trained_model_70epochs.pkl` — Pre-trained model file (generated by `train_70_epochs.py`)
- `training_history_70epochs.json` — Training metrics from 70-epoch training session
