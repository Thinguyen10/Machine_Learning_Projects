...
import pandas as pd
import numpy as np
# NLTK is optional for quick tests. Try to import, otherwise provide
# lightweight fallbacks so the rest of the pipeline can run for simple
# experiments without installing heavy NLTK packages.
try:
    import nltk
    ...
    # minimal tokenizer fallback (keeps words and apostrophes)
    import re as _re
    ...
    # fallback stopwords: provide an object with `.words()` like NLTK's interface
    class _FallbackStopwords:
        @staticmethod
        def words(lang='english'):
            return set(['the', 'and', 'is', 'in', 'it', 'of', 'to', 'a'])

    stopwords = _FallbackStopwords()

    # very small lemmatizer fallback (no real lemmatization)
...
# ------------------ Ensure NLTK data is available (if installed) ------------------
if _NLTK_AVAILABLE:
    try:
        _ = word_tokenize("test")
        _ = stopwords.words("english")
    except LookupError:
        # attempt to download required corpora
        try:
            nltk.download("punkt")
            ...
            # if downloads fail, we'll continue — fallback tokenizers exist
            pass
